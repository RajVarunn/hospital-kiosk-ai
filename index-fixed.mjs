...
export const handler = async (event, context) => {
  ...
  // Handle GET request for visits
  if (event.httpMethod === 'GET' && event.path.includes('/visits')) {
    return await handleGetVisits(event);
  }
...
/**
 * Handle GET requests for visits
 */
async function handleGetVisits(event) {
  try {
    // Check if there's a patient_id query parameter
    ...
    if (patientId) {
      ...
      // Get all visits
      return successResponse(await getVisits());
    }
  } catch (error) {
    console.error('Error handling GET visits:', error);
    return errorResponse(error.message);
  }
}
